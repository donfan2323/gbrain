...
**日付**: 2026-08-01(初版) / 2026-08-01(v2、外部レビューREQUIRED-2/6/7対応版) / 2026-08-02(v3、内部並列レビュー(Migration Reviewer)対応版) / 2026-08-02(v4、Migration Reviewer再レビューでの追加指摘対応版)
...
- `src/core/oauth-provider.ts` — `verifyAccessToken`の principal 関連JOIN・フォールバック分岐・返り値フィールドを削除(v2で追加した`isUndefinedTableError`のnarrowing呼び出しも含む)
