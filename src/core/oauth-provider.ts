...
export class GBrainOAuthProvider implements OAuthServerProvider {
  ...
  async verifyAccessToken(token: string): Promise<SdkAuthInfo> {
    ...
    // in the SAME query (no extra round trip, same design goal as the
    // source_id/federated_read JOIN below). On a pre-v125 brain the
    ...
    try {
      oauthRows = await this.sql`
        SELECT t.client_id, t.scopes, t.expires_at, t.resource, c.client_name,
               c.source_id, c.federated_read, c.principal_id, pk.id AS principal_kind
        FROM oauth_tokens t
        LEFT JOIN oauth_clients c ON c.client_id = t.client_id
        LEFT JOIN principals p ON p.id = c.principal_id
        LEFT JOIN principal_kinds pk ON pk.id = p.kind_id
        WHERE t.token_hash = ${tokenHash} AND t.token_type = 'access'
      `;
    } catch (err0) {
      if (
        isUndefinedColumnError(err0, 'principal_id')
        // REQUIRED-4 remediation: narrowed to the two Phase 9B tables this
        // exact query references (principals / principal_kinds) instead of
        // any undefined-table error. An unrelated missing-table error (e.g.
        // oauth_tokens/oauth_clients themselves absent — a brain broken far
        // beyond "just pre-Phase-9B") now hits `throw err0` below directly
        // instead of being silently absorbed here and retried one level in.
        || isUndefinedTableError(err0, 'principals')
        || isUndefinedTableError(err0, 'principal_kinds')
