...
CREATE TABLE IF NOT EXISTS oauth_clients (
  ...
  -- ON DELETE RESTRICT (not SET NULL): Principal rows are never physically
  -- deleted by design, so this only matters if that invariant is ever
  -- violated — RESTRICT rejects the delete outright rather than silently
  -- severing the audit-attribution reference. Same rationale as source_id
  -- above (see its own RESTRICT history in migrate.ts).
