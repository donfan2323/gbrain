  ...
  private async applyForwardReferenceBootstrap(): Promise<void> {
    ...
    const { rows } = await this.db.query(`
      SELECT
        ...
        EXISTS (SELECT 1 FROM information_schema.tables
                WHERE table_schema='public' AND table_name='principal_kinds') AS principal_kinds_exists,
        EXISTS (SELECT 1 FROM information_schema.tables
                WHERE table_schema='public' AND table_name='principals') AS principals_exists,
        EXISTS (SELECT 1 FROM information_schema.columns
                WHERE table_schema='public' AND table_name='oauth_clients' AND column_name='principal_id') AS oauth_clients_principal_id_exists,
        EXISTS (SELECT 1 FROM information_schema.tables
    ...
    // in PGLITE_SCHEMA_SQL references oauth_clients.principal_id, which in turn
    ...
    // the two new tables + the column before PGLITE_SCHEMA_SQL replay creates
    // the index; v125 runs later via runMigrations and is idempotent (all
    ...
    if (needsPrincipalIdBootstrap) {
      ...
      // idx_oauth_clients_principal_id in PGLITE_SCHEMA_SQL crashes on
      // pre-v125 brains without these. Bootstrap mirrors the v125 shape;
      // v125 runs later via runMigrations and is idempotent (IF NOT EXISTS /
      // ON CONFLICT DO NOTHING throughout, matching migrate.ts's version-125
      // entry).
      await this.db.exec(`
