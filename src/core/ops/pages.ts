...
const put_page: Operation = {
  ...
  handler: async (ctx, p) => {
    ...
    if (ctx.remote !== false && !trustedWorkspace) {
      // #4525: say WHY and what to do about it — a bare {skipped: 'remote'}
      // left agents believing their body wikilinks had been reconciled into
      // the graph when the remote-auto-link/timeline feature flags (below)
      // are off (the default).
      ...
      autoLinks = { skipped: 'remote', hint: remoteSkipHint };
      autoTimeline = { skipped: 'remote', hint: remoteSkipHint };
