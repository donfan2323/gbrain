...
export class PostgresEngine implements BrainEngine {
  ...
  async getBacklinkCounts(slugs: string[]): Promise<Map<string, number>> {
    ...
    //
    // v0.43-port (remote auto-link threat model, dashboard-h0cfe): also
    // filter 'remote-auto' — links created by put_page's remote-caller
    // opt-in path (ops/pages.ts runAutoLink linkSourceTag). These still
    // count toward link_count/orphan-reduction/graph traversal
    // (getLinks/getBacklinks), just not toward this ranking boost — an
    // untrusted remote page must not be able to inflate an existing page's
    // search ranking by planting a bare-slug mention of it. Same NULL-safe
    // `IS DISTINCT FROM` pattern.
    const sql = this.sql;
    const rows = await sql`
