 ...
 *  10. Deleting a Principal does not corrupt audit references (ON DELETE SET NULL,
 *      not CASCADE — the Client row and its audit trail survive)
 */
...
describe('oauth_clients.principal_id', () => {
  ...
  test('#10: deleting a linked principal sets principal_id to NULL, not the client row', async () => {
    const [{ id: principalId }] = await sql`INSERT INTO principals (kind_id, display_name) VALUES ('service', 'to-be-deleted') RETURNING id`;
    const { clientId } = await provider.registerClientManual(
      'principal-cascade-test', ['client_credentials'], 'read',
    );
    await sql`UPDATE oauth_clients SET principal_id = ${principalId} WHERE client_id = ${clientId}`;
    await sql`DELETE FROM principals WHERE id = ${principalId}`;
    const rows = await sql`SELECT client_id, principal_id FROM oauth_clients WHERE client_id = ${clientId}`;
    expect(rows.length).toBe(1); // client survives
    expect(rows[0].principal_id).toBeNull(); // reference cleared, not cascaded
  });
});
