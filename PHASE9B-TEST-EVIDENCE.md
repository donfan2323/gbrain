...
**日付**: 2026-08-01(初版) / 2026-08-01(v2、外部レビューREQUIRED-1〜7対応版) / 2026-08-01(v3、v2再レビューでのREQUIRED-1(フルUnit回帰の未解消分)対応版) / 2026-08-02(v4、v3再レビューでのREQUIRED-1(direct実行特有の追加2失敗+brain-repo-durability再現性)対応版) / 2026-08-02(v5、外部レビュー承認前の内部並列レビュー(7専門Reviewer)対応版) / 2026-08-02(v6、§11修正への再レビュー(第2ラウンド)対応版) / 2026-08-02(v7、外部レビュー「未承認・REQUIRED 3件」への対応版。§13・§14参照)
...
**v5内部レビュー対応**: 外部レビュー承認前にユーザー指示で実施した内部並列レビュー(7専門Reviewer)がREQUIRED合計28件(**⚠️v7訂正**: 当初「24件超」と記載していたが、Performanceの配送遅延分1件を含めた正確な内訳は`PHASE9B-IMPLEMENTATION-REPORT.md`§11の表を参照)を発見、全て対応 — ✓完了(§11参照)。§3-9関連の記述訂正(算術ミス・生ログ不一致等)も本ラウンドで実施した。
...
## 6. git status --short(v7再取得、循環参照を解消)

**⚠️v7で訂正**: 本節は当初「v2最終」のまま更新されず、その後v5でも部分的な訂正注記を追加しただけで実際の`git status --short`を再取得していなかった。加えてその訂正注記自体が`test/e2e/postgres-bootstrap.test.ts`を「新規`??`」と誤記していた(実際は既存ファイルへの変更で`M`)。さらに`test/minions-shell.test.ts`(v3で隔離バグ修正、`M`)が訂正注記からも完全に漏れていた。`PHASE9B-IMPLEMENTATION-REPORT.md`の「変更禁止領域について」段落(§5末尾)側も「本節(TEST-EVIDENCE§6)を参照(要再取得)」と本節に委ねたままで、2文書が互いを正本として指し合う循環参照になっていた(Documentation Reviewer v7ラウンドで指摘)。以下は実際に再取得した現在値であり、本節を正とする(循環参照を解消)。
...
```
 ...
 M src/core/utils.ts
 M src/schema.sql
 M test/e2e/postgres-bootstrap.test.ts
 M test/minions-shell.test.ts
...
本番ソースコード+テストの変更は14件の`M`(全てPhase 9Bのスコープ内ファイル。`src/commands/serve-http.ts`・`src/core/scope.ts`はv5のREQUIRED-Bで新たにM対象へ加わった。`test/e2e/postgres-bootstrap.test.ts`はv5のREQUIRED-Iで、`test/minions-shell.test.ts`はv3の隔離バグ修正で、それぞれ既存ファイルへの変更としてM対象へ加わった)。それ以外は全て新規`??`ファイル(Phase 9A文書7件+Phase 9B報告書5件+`PHASE9C-PREREQUISITES.md`1件+新規テスト6件)。`管理画面`・`.claude/settings*`への変更はゼロ(無変更を維持)。`src/commands/serve-http.ts`のみ、v5で内部レビューの発見を受け最小限(認可判断2行を関数呼び出しへ置換する挙動不変の移動のみ)の変更を行った(理由は§11 REQUIRED-B参照)。
...
1. `GBRAIN_PGLITE_SNAPSHOT`旧方式記述の残置4箇所 → 全て「§12訂正」注記付きで新方式(`database_path`)へ言及するよう修正済み(`grep -n "delete process.env.GBRAIN_PGLITE_SNAPSHOT" PHASE9B-*.md`のヒットを全て確認、いずれも訂正注記か旧記述の文脈内引用のみで、無条件の現状記述としての残置はゼロ。**⚠️v7注記**: この本文自体が同文字列を含むため、以後の編集が進むほどgrepのヒット数自体は増える。件数は継続的な編集で必ずドリフトするため以後この節では記載しない — 判定すべきは「無条件の現状記述が存在するか」のみで、それは一貫してゼロ)。
...
### 13-4. 未解消のまま残る唯一の限界(v7で解消)

**review-documentation・review-performanceからの「修正後REQUIRED=0」というライブでの最終承認は、セッション上限のため得られていない。** 上記13-1〜13-3のセルフ検証で客観的根拠は揃えたが、これは独立第三者レビュアーによる承認とは性質が異なる。ユーザーには、この制約を最終完了報告で明示する。**⚠️v7更新**: この状態でv6を外部監査(ChatGPT)に提出したところ「未承認・REQUIRED 3件」の判定を受けた。ユーザー指示により、v7ラウンドで新規独立エージェント(review-documentation-v7・review-performance-v7)を割り当て直し、ライブでの最終確認を取得した(詳細は§14)。当初の「翌朝7:50am以降に元エージェントへ再依頼」という想定は、新規エージェントへの再割当という形で実質的に解消された。
...
v6のReview Bundle(`PHASE9B-PRINCIPAL-FOUNDATION-REVIEW-BUNDLE-v6.zip`)を外部レビュー(ChatGPT)に提出したところ、「未承認・REQUIRED 3件」の判定を受けた。ユーザー指示により、Sonnet本体は統合・最終品質保証に専念し、以下4エージェントを並列委譲した:

| Agent | 担当 | 対応する外部監査REQUIRED |
|---|---|---|
| review-documentation-v7 | Documentation最終独立レビュー | REQUIRED-2(独立最終確認の取得) |
| review-performance-v7 | Performance最終独立レビュー | REQUIRED-2(同上) |
| regression-v7 | 最終v6/v7フル回帰とbaseline比較 | REQUIRED-3(フル回帰再実行+baseline機械比較) |
| consistency-audit-v7 | Beads・報告書・Bundle整合性監査 | REQUIRED-1(Beads状態と報告書の一致) |

各エージェントには、担当範囲・権限・完了条件を明示し、「未確認範囲が発生した場合は完了扱いにせず明示的にエスカレーションする」ことを義務付けた。

### 14-1. REQUIRED-1(Beads/報告書の状態矛盾)への対応

根本原因: 前ラウンドでBeads `dashboard-vyyod`を`CLOSED`に更新した後、Review Bundleの`COMPLETION-REPORT.md`・`README.md`は更新後の状態(CLOSED)を記載したが、`beads-task-info.txt`は更新**前**に取得したスナップショットのままバンドルへ同梱されており、バンドル内部で自己矛盾していた。

対応: Beads `dashboard-vyyod`を`IN_PROGRESS`/`REVIEW_PENDING`へ即座に差し戻し(外部承認取得までCLOSEDにしない、ユーザー明示指示)。その後consistency-audit-v7が独立監査を実施し、7件の追加の不整合(Beads記述以外にも、`git-diff-tracked-files.txt`の3ファイル欠落・`changed-files/`への31個の不正ディレクトリ混入・「24件超」表記の残置3箇所・第2ラウンドREQUIRED件数の算術不整合・`reports/`/`changed-files/`の内訳不一致2件)を発見、全て対応した(§13-2参照、および本書§0〜§13の各所)。v7のBundle再構築時は、これら全てを解消した状態で作成する(bash変数のクォート徹底・rm -rfしてから再構築・git diff対象ファイルリストの単一の真実源からの生成、を徹底)。

### 14-2. REQUIRED-2(Documentation/Performance独立最終確認)への対応

**review-documentation-v7**: 新規独立エージェントとして、CLOSED等の虚偽主張の有無・v5〜v13の訂正の波及漏れを全面的に再検証した。3往復のやり取りで計14件の指摘(「24件超」残置3箇所・`scope.ts`のgit diff空主張の陳腐化・git status循環参照・ロールバック手順書の対象ファイル欠落・認可マトリクスの陳腐化した数値と誤参照・実コードと設計文書の差異が3点目まである点・存在しない節への参照2件・行番号引用のドリフト複数件)を発見、全て修正確認済み。**最終判定: PASS(REQUIRED=0)。**

**review-performance-v7**: 新規独立エージェントとして、REQUIRED-1(冗長JOIN削除)の正しさを実Postgresの`EXPLAIN (VERBOSE)`実行込みで再検証(Nested Loop Left Join×2、Seq Scanゼロ、3本ともPK index probeであることを確認)。新規スイープでも追加REQUIREDなし。副次的に、`dashboard-m1ja3`(RLS非対称性)の前提が実機では既に成立していない可能性(`auto_enable_rls()`イベントトリガにより新旧テーブルとも同一のRLS状態)を発見・報告(Notes扱い、DB/セキュリティ担当への申し送りとして維持)。**最終判定: PASS(REQUIRED=0)。**

### 14-3. REQUIRED-3(フル回帰+baseline機械比較)への対応

regression-v7が、一時worktree(`v7-patched`ブランチ、検証専用コミット`e7236677fd7747fca8dafb6289a450078dc061e2`)にPhase 9Bの全変更(M14+新規19、cmpで実リポジトリと0件不一致を確認)を適用し、同一マシン・同一環境でpatched/baseline双方のフルスイートを直列実行(patched: 21:54:35Z–22:49:24Z、baseline: 22:50:21Z–23:45:10Z、各55分)して機械比較した。**総合判定: PASS(REQUIRED=0)。**

ユーザー指定の6合格基準、全て達成:

1. ✅ patched固有の未説明失敗ゼロ — `(file, test-name)`ペアの集合差分: A-only(patched)=0、B-only(baseline)=0、共通6件
2. ✅ Phase 9B関連テスト全件PASS — 6ファイル85 pass/0 fail/377 expect、OAuth回帰116 pass/0 fail/460 expect(両側バイト一致)、Postgres e2e 19 pass/0 fail/66 expect
3. ✅ full suite完走 — 全8shard(patched 4+baseline 4)が完了行を出力、割当ファイル数=実行ファイル数を全shardで確認(patched計1021ファイル、baseline計1016ファイル)。serialも両側90件の完了行
4. ✅ baseline由来の既存FAILを同一条件で直接証明 — 共通6件全てbaseline側でも今回の実行で再現(旧ログ流用なし)
5. ✅ rc=143を推測扱いしない — ラッパーを経由せず直接exit code取得(patched 1/1/1/1、baseline 0/1/1/1)。**143は一度も発生せず**、rc=143の正体は`run-unit-parallel.sh`の`SHARD_TIMEOUT=1500`(25分)が実測shard時間(2568〜3288秒=43〜55分)より短く、正常進行中のshardを`gtimeout`がSIGTERMで強制終了していたことと特定した(Beads`dashboard-ateb8`へ登録、Phase 9B非起因の既存インフラ課題)
6. ✅ check-test-isolationの新規違反ゼロ — 違反1件(`test/put-page-remote-auto.test.ts`)のみ、両側で出力バイト一致、Phase 9Bファイルは0件

**共通6件の失敗(いずれもPhase 9B非起因、両側で再現)**:
- `test/mcp-eval-capture.test.ts` ×2 — 同一bunプロセス内の他ファイルとの環境変数競合(`OPENAI_API_KEY`)。既知の隔離違反と同系統
- `test/page-search-vector-overflow.test.ts` ×1 — 高負荷下での30秒per-testタイムアウト超過。既知のflakeリスト該当
- `test/core/retry.test.ts` ×1 — `BATCH_AUDIT_SITES`の期待値が`mcp.put_page.remote_auto`を欠落。Phase 9B以前からのソース/テストドリフト(`dashboard-2vepx`へ登録)
- `test/scripts/run-unit-parallel.test.ts` ×2 — ラッパー自身の自己テスト。Phase 9Bは`scripts/`配下を一切変更していない(`dashboard-nc8tl`へ登録)

件数差分: patched(14153 unit実行/1021ファイル)− baseline(14077 unit実行/1016ファイル)= +76テスト/+5ファイル。Phase 9Bの新規テストファイル5件(patched shardに算入された分)と正確に一致。serial失敗は両側とも同一10件(hybrid-meta/autocut-integration/hybrid-reranker-integration/worker-registry、既知のflakeリストに該当)。

**未確認範囲(regression-v7の正直な開示)**: (a) 実行中にリアルタイムで本書等のドキュメント4件が編集されたため、regression-v7は`src/`/`test/`のドリフトのみ再確認(0件)、ドキュメント自体は本フル回帰の対象外。(b) `test:slow`・`run-e2e.sh`全体・`check-test-isolation`以外の`check:*`スクリプトは範囲外のため未実行。(c) baseline側e2eは`postgres-bootstrap`のみ実行、e2e全体は未実行。(d) 共通6件は「同時に発生する」ことは証明したが「互いに独立である」ことまでは証明していない(うち2件は負荷依存のため別環境では変動しうる)。

生ログは`/private/tmp/claude-501/-Users-lab2-AI-AI-1/21de68db-bdfd-4c14-841e-afde783c513b/scratchpad/phase9b-v7-regression-logs/`に保存済み(shard×8・serial×2・release×4・typecheck×2・check-test-isolation×2・Phase 9B専用/OAuth回帰/Postgres e2e等)。v7 Review Bundleの`test-logs/`へ移設する。

### 14-4. 総括

外部監査の3件全て、独立エージェントによる検証を経て解消した。REQUIRED-1(Beads/報告書矛盾)は根本原因(スナップショットの取得タイミング問題)を特定し再発防止策(bundle再構築時の手順見直し)込みで対応、REQUIRED-2(Documentation/Performance)はライブでの独立REQUIRED=0確認を取得、REQUIRED-3(フル回帰)はbaseline機械比較込みでPASSを確認した。**ただし、これは内部レビュー(Sonnet統合+4独立エージェント)による検証であり、外部レビュー(ChatGPT)自身による再承認ではない。** v7 Bundleを再提出し、外部レビューによる正式承認を得るまで、Phase 9Bは正式完了として扱わない。
